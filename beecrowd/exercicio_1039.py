...
Para obter uma flor de fogo, 
a pessoa deveria procurá-la em uma floresta antes da meia-noite na véspera do Noc Kupały. 
Exatamente à meia-noite ela floresceria. Para colhê-la seria preciso desenhar um círculo em volta dela. 
Parece uma tarefa fácil, no entanto, 
os espíritos malignos que guardam a flor tentariam de tudo para distrair qualquer um tentando colher a flor. 
...
try:
    while True:
        ...
        R1, X1, Y1, R2, X2, Y2 = map(int, entrada.strip().split())
        
        dx = X1 - X2
        dy = Y1 - Y2
        distancia_quadrado = dx * dx + dy * dy
        diferenca_raio = R1 - R2

        # se R1 < R2, já é impossível conter o outro círculo
        if diferenca_raio < 0:
            print("MORTO")
        elif distancia_quadrado <= diferenca_raio * diferenca_raio:
            print("RICO")
        else:
            print("MORTO")
